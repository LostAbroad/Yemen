...
var popup = L.popup({
    ...
    closeButton: true,
    autoClose: true
    }).setLatLng(map.getBounds().getCenter()).setContent('<p class= mainPopup1><b>Layer Selection:</b> The top right corner of this map contains various layers that can be added to the map. If you are interested in how each  layer collected/made decisions on classifications click on the "?" on the navigation bar above.</p>                         <p class= mainPopup2><b>Food Insecurity:</b> Shows forward-looking analysis representing the most likely food security for the near term. Not having access to reliable food can lead to malnutrition, death, an increase in civilian conflicts, and more. </p>                                                                                 <p class= mainPopup3><b>Livelihood Zone:</b>This is included to give context and potential reasons why Yemen is currently suffering. For example, areas with more air strikes are more likely to have lower crop yields from being felled. </p>                 <p class= mainPopup4><b>Faction:</b>This layer is included to show how each faction is doing in that regard and to compare the types of conflicts taking place in each zone. </p>                                                                                       <p class= mainPopup5><b>Population:</b> This layer is meant to help showcase the role population has to play in the level of food insecurity and the types of conflicts within Yemen. </p>                                                                               <p class= mainPopup6><b>Conflicts:</b>This layer is meant to allow for an easier way to visually see how each conflict may be impacting Yemen on its road to recovery.</p>     <p class= mainPopup7><b>Boundaries:</b> County, State, and County boundaries have been included to serve as a reference.</p>').openOn(map);
