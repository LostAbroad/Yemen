...
var popup = L.popup({
    keepInView: true,
    offset: L.point(30,203),
    closeButton: true,
    autoClose: true
    }).setLatLng(map.getBounds().getCenter()).setContent('<p class= mainPopup1 style="font-size:(3px + 2vw)"><b><a href="https://lostabroad.github.io/YemenSplit/">Split Screen View: Computer Recommended</a></b><br><b>Layer Selection:</b> The top right corner of this map contains various layers that can be added to the map. If you are interested in how each  layer collected/made decisions on classifications click on the "?" on the navigation bar above.<br><b>Food Insecurity:</b> Shows forward-looking analysis representing the most likely food security for the near term. Not having access to reliable food can lead to malnutrition, death, an increase in civilian conflicts, and more. <br><b>Livelihood Zone:</b>This is included to give context and potential reasons why Yemen is currently suffering. For example, areas with more air strikes are more likely to have lower crop yields from being felled.<br><b>Faction:</b>This layer is included to show how each faction is doing in that regard and to compare the types of conflicts taking place in each zone.<br><b>Population:</b> This layer is meant to help showcase the role population has to play in the level of food insecurity and the types of conflicts within Yemen. <br><b>Conflicts:</b>This layer is meant to allow for an easier way to visually see how each conflict may be impacting Yemen on its road to recovery.<br><b>Boundaries:</b> County, State, and County boundaries have been included to serve as a reference.</p>').openOn(map);
